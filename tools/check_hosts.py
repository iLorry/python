...
def create_parser(filename):
    ...
    parser = argparse.ArgumentParser(
        description='Check the hosts status.', prefix_chars='-/')
...
def check_host(host, url, signal, user, password, timeout=1):
    semaphore.acquire()

    try:
        res = requests.get(url, timeout=timeout, auth=(user, password))
        status = res.status_code
        if (status == 200) and (res.text.strip() == signal):
            status = 'ok'
        print('[{0}] {1}'.format(status, host))
    except:
